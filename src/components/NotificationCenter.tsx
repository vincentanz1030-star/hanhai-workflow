...
export function NotificationCenter({
  ...
  return (
    <Card className="border-0 shadow-sm overflow-hidden h-full flex flex-col">
      {/* 顶部标题栏 */}
      <div className="flex items-center justify-between px-3 py-2.5 bg-gradient-to-r from-amber-500 to-orange-500 text-white shrink-0">
      ...
      <div className="flex-1 grid grid-cols-2 divide-x divide-border min-h-0">
        ...
        <div className="flex flex-col min-h-0">
          <div className="flex items-center justify-between px-2.5 py-1.5 bg-amber-50/80 dark:bg-amber-950/30 border-b shrink-0">
            <div className="flex items-center gap-1.5">
              <Megaphone className="h-3 w-3 text-amber-600" />
              <span className="font-medium text-xs">公告</span>
              <Badge variant={unreadAnnouncements > 0 ? "destructive" : "secondary"} className="text-[9px] h-3.5 px-1">
                {totalAnnouncements}
              </Badge>
            </div>
            {isAdmin && (
              <Button size="sm" variant="ghost" className="h-5 w-5 p-0" onClick={handleAddAnnouncement}>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start gap-2">
                        <div className={cn('p-0.5 rounded text-white shrink-0', config.bgColor)}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1">
                            ...
                            {!announcement.isRead && (
                              <Badge variant="secondary" className="text-[8px] h-3 px-0.5">新</Badge>
                            )}
                          </div>
                          {announcement.content && (
                            <p className="text-[10px] text-muted-foreground line-clamp-2 mt-0.5">{announcement.content}</p>
                          )}
                        </div>
                        {isAdmin && (
        ...
        {/* 右侧：通知区 */}
        <div className="flex flex-col min-h-0">
          <div className="flex items-center justify-between px-2.5 py-1.5 bg-blue-50/80 dark:bg-blue-950/30 border-b shrink-0">
            <div className="flex items-center gap-1.5">
                        ...
                        <div className="flex items-start gap-2">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1 flex-wrap">
                              ...
                              <Badge className={`${priorityColor} text-[8px] h-3 px-0.5`}>
                                {priority === 'high' ? '紧急' : priority === 'medium' ? '重要' : '普通'}
                              </Badge>
                              {isOverdue && (
                                <Badge variant="destructive" className="text-[8px] h-3 px-0.5">逾期</Badge>
                              )}
                            </div>
                            <p className="text-[10px] text-muted-foreground line-clamp-1 mt-0.5">{notification.content}</p>
                          </div>
