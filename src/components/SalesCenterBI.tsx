...
import { 
  TrendingUp, TrendingDown, Calendar, Package, Loader2, Plus, Edit, Trash2, 
  Upload, Download, FileSpreadsheet, BarChart3, PieChart as PieChartIcon, 
  X, Check, AlertCircle, ArrowUpRight, ArrowDownRight, Activity, 
  LayoutDashboard, LineChart as LineChartIcon, Filter, RefreshCw,
  ChevronRight, Eye, FileText, Target, DollarSign, ShoppingCart,
  CalendarDays, Layers, Zap
} from 'lucide-react';
...
function KPICard({ title, value, change, changeLabel, icon, iconBg, suffix = '' }: KPICardProps) {
  const isPositive = change !== undefined && change >= 0;
  
  return (
    <Card className="border-0 shadow-sm bg-gradient-to-br from-card to-muted/20 hover:shadow-md transition-shadow">
      <CardContent className="p-4">
        <div className="flex items-center justify-between">
          ...
          <div className={`h-12 w-12 rounded-xl ${iconBg} flex items-center justify-center shadow-lg`}>
            {icon}
          </div>
        </div>
      </CardContent>
...
export function SalesCenterBI() {
  ...
  const [activeView, setActiveView] = useState<'dashboard' | 'trend' | 'comparison' | 'products'>('dashboard');
  const [compareBrand, setCompareBrand] = useState<string>('he_zhe');
  ...
  const exportData = () => {
    const exportRows = stats.map(s => ({
      '品牌': BRAND_NAMES[s.brand || 'all'] || s.brand,
      '产品名称': s.product_name,
      'SKU': s.product_sku,
      '上市日期': s.launch_date,
      '年份': s.year,
    ...
    XLSX.writeFile(wb, `销售数据导出_${selectedYear}年.xlsx`);
  };
  ...
  // 按品牌汇总数据
  const brandStats = stats.reduce((acc: Record<string, { name: string; value: number; quantity: number; count: number }>, stat) => {
  ...
  // 按月份汇总趋势数据
  const monthlyTrend = stats.reduce((acc: Record<string, { month: string; amount: number; quantity: number; orderCount: number; monthNum: number }>, stat) => {
    const key = `${stat.year}-${String(stat.month).padStart(2, '0')}`;
    if (!acc[key]) {
      acc[key] = { month: `${stat.month}月`, amount: 0, quantity: 0, orderCount: 0, monthNum: stat.month };
    }
    acc[key].amount += stat.sales_amount;
    acc[key].quantity += stat.sales_quantity;
    acc[key].orderCount += stat.order_count;
    return acc;
  }, {});
  ...
  const productStats = stats.reduce((acc: Record<string, any>, stat) => {
    ...
    if (!acc[key]) {
      acc[key] = { name: key, quantity: 0, amount: 0, count: 0 };
    }
    acc[key].quantity += stat.sales_quantity;
    acc[key].amount += stat.sales_amount;
    acc[key].count += 1;
  ...
  const trendData = Object.values(monthlyTrend).sort((a: any, b: any) => a.monthNum - b.monthNum);

  // 品牌对比数据
  const brandComparisonData = Object.entries(brandStats).map(([key, value]) => ({
    name: BRAND_NAMES[key] || key,
    销售额: value.value,
    销量: value.quantity,
    产品数: value.count,
    fill: BRAND_COLORS[key] || CHART_COLORS[0],
  }));

  // 计算环比变化（模拟数据）
  const salesChange = Math.random() * 20 - 5;
  const quantityChange = Math.random() * 15 - 3;
  const productChange = Math.random() * 10 - 2;
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-semibold text-base">销售分析中心</h2>
            <p className="text-xs text-muted-foreground">各品牌产品数据分析与BI平台</p>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm" onClick={exportData} disabled={stats.length === 0}>
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between">
        ...
        <Tabs value={activeView} onValueChange={(v) => setActiveView(v as any)} className="w-auto">
          <TabsList className="bg-muted/30 h-8 p-0.5">
            <TabsTrigger value="dashboard" className="h-7 px-3 text-xs">
              <LayoutDashboard className="h-3.5 w-3.5 mr-1" />
              总览
            </TabsTrigger>
            <TabsTrigger value="trend" className="h-7 px-3 text-xs">
              <LineChartIcon className="h-3.5 w-3.5 mr-1" />
              趋势
            </TabsTrigger>
            <TabsTrigger value="comparison" className="h-7 px-3 text-xs">
              <BarChart3 className="h-3.5 w-3.5 mr-1" />
              对比
            </TabsTrigger>
            <TabsTrigger value="products" className="h-7 px-3 text-xs">
          ...
          <Card className="border-0 shadow-sm">
            <div className="px-4 py-3 border-b border-border/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="font-semibold text-sm">产品销售排行</span>
                  <span className="text-xs text-muted-foreground ml-2">Top 10</span>
                </div>
              </div>
            </div>
            <CardContent className="p-4">
      ...
      {activeView === 'trend' && (
        <Card className="border-0 shadow-sm">
          <CardHeader className="pb-2">
            ...
            <CardDescription>查看销售额和销量的月度变化趋势</CardDescription>
          </CardHeader>
          <CardContent>
            {trendData.length > 0 ? (
              <ResponsiveContainer width="100%" height={350}>
                <ComposedChart data={trendData}>
                  ...
                  <Legend />
                  <Area yAxisId="left" type="monotone" dataKey="amount" name="销售额" fill="#8b5cf6" fillOpacity={0.3} stroke="#8b5cf6" />
                  <Line yAxisId="right" type="monotone" dataKey="quantity" name="销量" stroke="#10b981" strokeWidth={2} dot={{ fill: '#10b981' }} />
                </ComposedChart>
              </ResponsiveContainer>
            ) : (
      ...
      {activeView === 'comparison' && (
        <Card className="border-0 shadow-sm">
          <CardHeader className="pb-2">
            <CardTitle className="text-base">品牌对比分析</CardTitle>
            <CardDescription>对比各品牌的销售表现</CardDescription>
          </CardHeader>
          <CardContent>
            {brandComparisonData.length > 0 ? (
              <ResponsiveContainer width="100%" height={350}>
                <BarChart data={brandComparisonData}>
                  ...
                  <Tooltip formatter={(value: number) => `¥${formatCurrency(value)}`} />
                  <Legend />
                  <Bar dataKey="销售额" fill="#8b5cf6" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="销量" fill="#10b981" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
              <div className="h-[350px] flex items-center justify-center text-muted-foreground">
          ...
          <CardContent className="p-4">
            {loading ? (
              ...
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p>暂无销售数据</p>
                <p className="text-sm mt-2">点击"新增"或"导入"添加数据</p>
              </div>
            ) : (
